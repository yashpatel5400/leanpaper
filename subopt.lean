...
noncomputable def Delta_test {Ω 𝓧 𝓒 𝓦 E : Type*} [PseudoEMetricSpace 𝓒] [PseudoEMetricSpace E]
  (w : 𝓦) (f : 𝓦 → 𝓒 → E) (U : 𝓧 → Set 𝓒) (X : Ω → 𝓧) (C : Ω → 𝓒) (ω : Ω) : ENNReal :=
...
theorem test_vol (s : Set Ω) : volume s = volume s := rfl

theorem test_ineq (s : Set Ω) (α : ENNReal) : volume s ≥ 1 - α ↔ volume s ≥ 1 - α := Iff.rfl
